...
interface UiModalComponentsPropsI {
  className?: string;
  children: ReactNode;
}

interface CrossLightIconPropsI {
  className: string;
}
...
export function UiModal({
  ...
  const modal = (
    <div
      onClick={handleClick}
      className="fixed inset-0 bg-slate-900/60 backdrop-blur pt-10 pb-10 overflow-y-auto"
    >
      <div
        data-id="modal"
        className={clsx(
...
function CrossLightIcon({ className }: CrossLightIconPropsI) {
  return (
    <svg
      className={className}
      width="16"
